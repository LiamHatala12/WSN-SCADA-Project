/*
  ESP-NOW Servo Actuator Node
  - Priority 2
  - Receives MSG_SERVO_COMMAND from HEAD and sets servo angle
*/
...
/* ESP-NOW globals */
uint32_t self_priority      = PRIORITY_SERVO;
...
public:
  ...
  void onReceive(const uint8_t *data, size_t len, bool broadcast) {
    ...
    if (msg->msg_type == MSG_SERVO_COMMAND) {
      int32_t servoDeg = msg->data;
      if (servoDeg < 0)   servoDeg = 0;
      if (servoDeg > 180) servoDeg = 180;

      Serial.printf("Received SERVO_COMMAND from HEAD " MACSTR ": %ld deg\n",
                    MAC2STR(addr()), (long)servoDeg);

      hs422.write((int)servoDeg);
    }
  }
...
void setup() {
  ...
  Serial.println("Servo node setup complete, discovering HEAD...");
}
...
void loop() {
  if (!master_decided) {
    ...
    if (current_peer_count == ESPNOW_PEER_COUNT && check_all_peers_ready()) {
      ...
      if (device_is_master) {
        Serial.println(">>> Servo became master (unexpected)");
      } else {
        Serial.println(">>> Servo is slave, HEAD is master");
        for (auto &peer : peers) {
